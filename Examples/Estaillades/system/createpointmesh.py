...
if (NPX*NPY*NPZ>1):
    for ipz in range(0,NPZ):
        for ipy in range(0,NPY):
            for ipx in range(0,NPX):
                ...
                f=open('processor'+str(iproc)+'/constant/polyMesh/points','a')
                ...
                for k in range (0,nzp+1):
                    z=round(ipz*nzp*resZ+zmin*res+k*resZ,9)
                    if (np.abs(z)<1e-9):
                        num_z='0'
                    else:
                        num_z=str(z)
                    for j in range (0, nyp+1):
                        y=round(ipy*nyp*resY+ymin*res+j*resY,9)
                        if (np.abs(y)<1e-9):
                            num_y='0'
                        else:
                            num_y=str(y)
                        for i in range (0, nxp+1):
                            x=round(ipx*nxp*resX+xmin*res+i*resX,9)
                            if (np.abs(x)<1e-9):
                                num_x='0'
                            else:
                                num_x=str(x)
                            f.write("("+num_x+" "+num_y+" "+num_z+")"+'\n')
                f.write(")"+'\n')
                ...
                f=open('processor'+str(iproc)+'/constant/polyMesh/pointProcAddressing','a')
                f.seek(0) #get to the first position
                ##So that the files are exactly the same than with blockMesh - remove later
                f.write('/*--------------------------------*- C++ -*----------------------------------*\\'+'\n')
                f.write('| =========                 |                                                 |'+'\n')
                ...
                f.write("    location    \"constant/polyMesh\";"+'\n')
                f.write("    object      pointProcAddressing;"+'\n')
                f.write("}"+'\n')
                ...
                for k in range (0,nzp+1):
                    for j in range (0, nyp+1):
                        for i in range (0, nxp+1):
                            f.write(str((ipz*nzp+k)*(ny1+1)*(nx1+1)+(ipy*nyp+j)*(nx1+1)+(ipx*nxp+i))+'\n')
                f.write(")"+'\n')
                ##So that the files are exactly the same than with blockMesh - remove later                
                f.write('\n')                
                f.write('\n')
    ...
    f=open('constant/polyMesh/points','a')
    f.seek(0) #get to the first position
    ...
    f.write(str(npoints)+'\n')
    f.write("("+'\n')
    for k in range (0,nz1+1):
        z=zmin+round(zmin*res+k*resZ,9)
        for j in range (0, ny1+1):
            y=round(ymin*res+j*resY,9)
            for i in range (0, nx1+1):
                x=round(xmin*res+i*resX,9)
