...
#Define image name
Image_name="Estaillades-Z2-14p_500x1000x1000_3p9676um"
...
#micro_por=('1' '0.57' '0.52' '0.47' '0.42' '0.36' '0.27' '0.22' '0.18' '0.15' '0.12' '0.09' '0.07' '0.0001' '0.0001')
...
micro_k=('7.5e11' '7.47e-15' '6.91e-15' '4.79e-15' '3.24e-15' '2.12e-15' '8.06e-16' '4.59e-16' '2.44e-16' '1.17e-16' '4.95e-17' '1.73e-17' '4.76e-18' '2e-26' '2e-26')
...
mpirun -np $NP python createMesh.py --xDim $x_dim --yDim $y_dim --zDim $z_dim --xMin $x_min --xMax $x_max --yMin $y_min --yMax $y_max --zMin $z_min --zMax $z_max --nX $n_x --nY $n_y --nZ $n_z --res $res --Image_name $Image_name --padWidth $padWidth --pores_value $pores_value --solid_value $solid_value --direction $direction --micro_por ${micro_por[@]} --micro_k ${micro_k[@]} --phases ${phases[@]} --NPX $NPX --NPY $NPY --NPZ $NPZ

echo -e "create eps"
python system/createEps.py --xDim $x_dim --yDim $y_dim --zDim $z_dim --xMin $x_min --xMax $x_max --yMin $y_min --yMax $y_max --zMin $z_min --zMax $z_max --nX $n_x --nY $n_y --nZ $n_z --Image_name $Image_name --padWidth $padWidth --pores_value $pores_value --solid_value $solid_value --direction $direction --micro_por ${micro_por[@]} --phases ${phases[@]} --NPX $NPX --NPY $NPY --NPZ $NPZ

echo -e "create Kinv"
python system/createKinv.py --xDim $x_dim --yDim $y_dim --zDim $z_dim --xMin $x_min --xMax $x_max --yMin $y_min --yMax $y_max --zMin $z_min --zMax $z_max --nX $n_x --nY $n_y --nZ $n_z --Image_name $Image_name --padWidth $padWidth --pores_value $pores_value --solid_value $solid_value --direction $direction --micro_k ${micro_k[@]} --phases ${phases[@]} --NPX $NPX --NPY $NPY --NPZ $NPZ

#rm -rf constant/triSurface

#mpiexec -np $NP topoSet -parallel > toposet.out
#mpiexec -np $NP subsetMesh solid -patch solidwalls0 -overwrite -parallel > subsetMesh.out
#mpiexec -np $NP createPatch -overwrite -parallel > createPatch.out
#sed -i "s/solidwalls0/solidwalls/g" processor*/0/eps
#sed -i "s/empty/zeroGradient/g" processor*/0/eps
#sed -i "s/solidwalls0/solidwalls/g" processor*/0/Kinv
#sed -i "s/empty/zeroGradient/g" processor*/0/Kinv
