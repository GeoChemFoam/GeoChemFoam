...
source /work/ecseaj02/ecseaj02/gavingcf/works/GeoChemFoam-v-dev/etc/bashrc
...
then
    ...
    echo "However, whilst NP=8, we run on 1 MPI task as the only script that createMesh.py runs, namely system/createblockmeshpar.py, is currently a serial task"
#    exit
fi

export OMP_NUM_THREADS=1
SRUN_CPUS_PER_TASK=${SLURM_CPUS_PER_TASK}
srun --distribution=block:block --hint=nomultithread python createMesh.py --xDim $x_dim --yDim $y_dim --zDim $z_dim --xMin $x_min --xMax $x_max --yMin $y_min --yMax $y_max --zMin $z_min --zMax $z_max --nX $n_x --nY $n_y --nZ $n_z --nLevel $nlevel --refineStokes $refineStokes --res $res --Image_name $Image_name --padWidth $padWidth --pores_value $pores_value --solid_value $solid_value --direction $direction --micro_por ${micro_por[@]} --micro_k ${micro_k[@]} --phases ${phases[@]}
